    ...
    export class ScrapManager {
      ...
      async processURL(url: string): Promise<ScrapResult> {
        try {
          ...
          const [domContent, htmlStructure, responseData] = await Promise.all([
            this.domScraper.scrapeContent(html),
            this.htmlScanner.scanHTML(html),
            this.httpParser.parseResponse(response)
          ]);
          ...
          this.eventBus.publish('scrap:error', { url, error: error.message });
